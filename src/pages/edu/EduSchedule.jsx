...
import { CalendarDays, ChevronLeft, ChevronRight, RefreshCw, Loader2, MapPin } from "lucide-react";
...
export default function EduSchedule() {
  ...
  function allDayForDate(d) {
    const k = localKey(d);
    const tasks = deliverables.filter((dl) => (dl.due_date || "") === k).map((dl) => ({ kind: "task", title: dl.title, color: taskHex(dl.type), isExam: ["exam", "midterm", "final"].includes(dl.type) }));
    ...
    <>
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 py-6 space-y-5">
        <div className="flex items-center justify-between gap-3 flex-wrap">
          ...
          <Button onClick={syncTasks} disabled={syncing} className="bg-emerald-500 text-black hover:bg-emerald-400 h-8">
            {syncing ? <Loader2 className="h-3.5 w-3.5 mr-1 animate-spin" /> : <RefreshCw className="h-3.5 w-3.5 mr-1" />} Sync to Google
          </Button>
        </div>
        ...
        {/* DAY */}
        {frame === "day" && <DayView date={anchor} blocks={[...courseBlocksForDate(anchor), ...gcalBlocksForDate(anchor)]} allDay={allDayForDate(anchor)} />}
        ...
        {frame === "week" && (
          <div className="overflow-x-auto">
            <div className="min-w-[720px] grid" style={{ gridTemplateColumns: "44px repeat(7, minmax(0,1fr))" }}>
              ...
              <HourAxis />
              {(() => { const s = startOfWeek(anchor); return DOW_FULL.map((_, i) => { const day = new Date(s); day.setDate(s.getDate() + i); return <DayColumn key={i} date={day} blocks={[...courseBlocksForDate(day), ...gcalBlocksForDate(day)]} allDay={allDayForDate(day)} />; }); })()}
            </div>
          </div>
        )}
        ...
        {selectedDay && (
          <DayPanel date={selectedDay} blocks={[...courseBlocksForDate(selectedDay), ...gcalBlocksForDate(selectedDay)]} allDay={allDayForDate(selectedDay)} onClose={() => setSelectedDay(null)} />
        )}
      </main>
      <EduBottomNav />
    </>
...
function DayColumn({ date, blocks, allDay }) {
...
function DayView({ date, blocks, allDay }) {
  return (
    <div className="rounded-lg border border-white/10 bg-black p-4">
      <div className="grid" style={{ gridTemplateColumns: "48px 1fr" }}>
        <HourAxis />
        <DayColumn date={date} blocks={blocks} allDay={allDay.length ? allDay : [{ kind: "gcal-all", title: "", color: "#000" }].filter(() => false)} />
      </div>
    </div>
  );
}
...
function DayPanel({ date, blocks, allDay, onClose }) {
  return (

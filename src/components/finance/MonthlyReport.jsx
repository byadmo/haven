...
export default function MonthlyReport() {
  ...
  const [year, setYear] = React.useState(String(prevYear));
  const [month, setMonth] = React.useState(String(prevMonthIdx + 1).padStart(2, "0"));
  ...
  async function generate() {
    ...
    try {
      const anchor = new Date(Number(year), Number(month) - 1, 1);
      ...
      doc.save(`monthly-summary-${year}-${month}.pdf`);
  ...
  return (
    <div className="rounded-lg border border-white/10 bg-black p-5">
      ...
      <div className="flex flex-col sm:flex-row gap-2">
        ...
        <Select value={year} onValueChange={setYear} disabled={busy}>
          <SelectTrigger className="bg-zinc-950 border-white/10 text-zinc-100 h-10 w-28">
            <SelectValue />
          </SelectTrigger>
          <SelectContent className="bg-zinc-900 border-zinc-800 max-h-72">
            {YEARS.map((y) => (
